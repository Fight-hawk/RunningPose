...
class DetectionLoader:
    ...
    def update(self):
        ...
        for i in range(self.num_batches):
            ...
            if img is None:
                # print('dataloader line 320 img is None')
            ...
            for k in range(len(orig_img)):
                ...
                if self.Q.full():
                    time.sleep(2)
                self.Q.put((orig_img[k], im_name[k], boxes_k, scores[np.argmax(scores)], inps, pt1, pt2))
    ...
    def update(self):
        ...
        for i in range(self.datalen):
            ...
            with torch.no_grad():
                ...
                if orig_img is None:
                    # print('dataloader line 409 orig_img is None')
                    self.Q.put((None, None, None, None, None, None, None))
                    return
...
class DataWriter:
    def __init__(self, save_video=False,
        ...
        if opt.save_img:
            ...
            if not os.path.exists(os.path.join(opt.outputpath, 'images')):
                os.makedirs(os.path.join(opt.outputpath, 'images'))
            if not os.path.exists(os.path.join(opt.outputpath, 'json')):
    ...
    def update(self):
        ...
        while True:
            ...
            if self.stopped:
                if self.save_video:
                    self.stream.release()
                    self.stream_black.release()
                return
            # otherwise, ensure the queue is not empty
            if not self.Q.empty():
                (boxes, scores, hm_data, pt1, pt2, orig_img, im_name) = self.Q.get()
                orig_img = np.array(orig_img, dtype=np.uint8)
                if orig_img is None or orig_img.shape[0] <= 200 or orig_img.shape[1]<=200:
                    continue
                if boxes is None:
                    pass
                    ...
                    # p_index = np.array([5, 6, 11, 12])
                    if opt.save_img:
                        for index, human in enumerate(result['result']):
                            json_obj = {}
                            kp_preds = np.array(human['keypoints'])
                            # print(human['kp_score'])
                            valid_index = np.where(np.array(human['kp_score'])>0.05)[0]
                            if 5 not in valid_index or 6 not in valid_index or 11 not in valid_index or 12 not in valid_index:
                                continue
                            left, right = min(int(kp_preds[5][0]),int(kp_preds[6][0]), int(kp_preds[11][0]),int(kp_preds[12][0])), max(int(kp_preds[5][0]),int(kp_preds[6][0]), int(kp_preds[11][0]),int(kp_preds[12][0]))
                            top, bottom = min(int(kp_preds[5][1]),int(kp_preds[6][1]), int(kp_preds[11][1]),int(kp_preds[12][1])), max(int(kp_preds[5][1]),int(kp_preds[6][1]), int(kp_preds[11][1]),int(kp_preds[12][1]))
                            top = top + (bottom - top) // 4
                            height = abs(top - bottom)
                            middle = (left + right) / 2
                            json_obj['lshouder'] = (int(min(middle + height / 2, orig_img.shape[1])), top)
                            json_obj['rshouder'] = (int(max(middle - height / 2, 0)), top)
                            json_obj['lhip'] = (int(min(middle + height / 2, orig_img.shape[1])), bottom)
                            json_obj['rhip'] = (int(max(middle - height / 2, 0),), bottom)
                            img = orig_img[top:bottom,
                                  int(max(middle - height / 2, 0)):int(min(middle + height / 2, orig_img.shape[1]))]
                            # img = self.vis_frame_fast(orig_img, json_obj)
                            if img is None:
                                continue
                            cv2.imwrite(os.path.join(opt.outputpath, 'vis',
                                                     result['imgname'].replace('.jpg', '_{}.jpg'.format(index))), img)
                            
                            json_obj['lshouder'] = (int(kp_preds[5][0]), top)
                            json_obj['rshouder'] = (int(kp_preds[6][0]), top)
                            json_obj['lhip'] = (int(kp_preds[11][0]), int(kp_preds[11][1]))
                            json_obj['rhip'] = (int(kp_preds[12][0]), int(kp_preds[12][1]))
                            
                            # if not os.path.exists(os.path.join(opt.outputpath, 'skin')):
                            #     os.makedirs(os.path.join(opt.outputpath, 'skin'))
                            # skin = self.skin_detector(orig_img[top:bottom, left:right], mode='ycrcb')
                            # if skin is None or np.mean(skin)/255 <= 0.6:
                            #     continue
                            # cv2.imwrite(os.path.join(opt.outputpath, 'skin', result['imgname'].replace('.jpg', '_{}.jpg'.format(index))), skin)
                            left, right = math.floor(np.min(kp_preds, axis=0)[0]), math.ceil(np.max(kp_preds, axis=0)[0])
                            bottom, top = math.ceil(np.max(kp_preds, axis=0)[1]), math.floor(np.min(kp_preds, axis=0)[1])
                            human_img = orig_img[top:bottom, left:right]
                            if human_img.shape[0]*human_img.shape[1] / (orig_img.shape[0]*orig_img.shape[1]) <= (1/3):
                                continue
                            if human_img.shape[0] == 0 or human_img.shape[1] == 0:
                                continue
                            # if not os.path.exists(os.path.join(opt.outputpath, 'json')):
                            #     os.makedirs(os.path.join(opt.outputpath, 'json'))
                            # cv2.imwrite(os.path.join(opt.outputpath, 'images', result['imgname'].replace('.jpg', '_{}.jpg'.format(index))), human_img)
                            with open(os.path.join(opt.outputpath, 'json', result['imgname'].replace('.jpg', '_{}.json'.format(index))), 'w', encoding='utf-8') as f:
    ...
    '''
    '''
    def vis_frame_fast(self, frame, json_obj):
        ...
        img = frame
        # black_img = np.zeros(shape=frame.shape, dtype=np.uint8)
        part_line = []
        for n, key in enumerate(json_obj.keys()):
            cor_x, cor_y = json_obj[key][0], json_obj[key][1]
            # part_line.append((cor_x, cor_y))
            cv2.circle(img, (cor_x, cor_y), 4, p_color[n], -1)
            # Draw limbs
        cv2.line(img, json_obj['lshouder'], json_obj['rshouder'], line_color[0], thickness=3)
        cv2.line(img, json_obj['rshouder'], json_obj['rhip'], line_color[1], thickness=3)
        cv2.line(img, json_obj['rhip'], json_obj['lhip'], line_color[2], thickness=3)
        cv2.line(img, json_obj['lhip'], json_obj['lshouder'], line_color[3], thickness=3)

        return img
    '''

    def skin_detector(self, img, mode='rgb'):
        '''
        检测腹部是否暴露
        :param img:
        :param threhold:
        ...
        if img.shape[0] >224 or img.shape[1] > 224:
            img = np.resize(img, new_shape=(224, 224, 3))
        # print('=======cl')
        # print(img.shape)
        if mode == 'rgb':
            skin = np.zeros(shape=img.shape, dtype=np.uint8)
            ...
            _, skin_ = cv2.threshold(cr1, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            for i in range(3):
                skin[:, :, i] = skin_
            return skin
        elif mode == 'ycrcb':
            skin = np.zeros(shape=img.shape, dtype=np.uint8)
            ycrcb = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)  # 把图像转换到YUV色域
            (y_c, cr_c, cb_c) = cv2.split(ycrcb)
            for i in range(img.shape[0]):
                for j in range(img.shape[1]):
                    y, cr, cb = y_c[i][j], cr_c[i][j], cb_c[i][j]
                    if (cr > 77 and cb < 127) and (cr > 133 and cr < 173):
                        skin[i, j, :] = 255
            return skin
        ...
        while True:
            ...
            if not self.Q.empty():
                ...
                if boxes is None:
                    ...
                    self.final_result.append(result)
                    if opt.save_img or opt.save_video or opt.vis:
                        img, black_img = vis_frame(orig_img, result)
                        if opt.vis:
                            cv2.imshow("AlphaPose Demo", img)
                            cv2.waitKey(30)
                        if opt.save_img:
                        ...
                        if opt.save_video:
                            self.stream.write(img)
                            self.stream_black.write(black_img)
            else:
                time.sleep(0.1)
    def running(self):
