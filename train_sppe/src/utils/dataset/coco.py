...
class Mscoco(data.Dataset):
    def __init__(self, train=True, sigma=1,
        ...
        self.img_folder = '/workspace/images/train2017'    # root image folders
        ...
        with h5py.File('../data/coco/annot_coco.h5', 'r') as annot:
            # train
            self.imgname_coco_train = annot['imgname'][:-300]
            self.bndbox_coco_train = annot['bndbox'][:-300]
            self.part_coco_train = annot['part'][:-300]
            # val
            self.imgname_coco_val = annot['imgname'][-300:]
            self.bndbox_coco_val = annot['bndbox'][-300:]
            self.part_coco_val = annot['part'][-300:]
